...
>
> **Last updated:** 2026-06-15 (**BG-7 zero-touch PROVEN on clone** — real POS Pay → auto WhatsApp; fixed a broken-assembly-name helper DLL crash; Meta "API access blocked" = personal-account verify — see LRN-2026-06-15-*)
...
### LRN-2026-07-03-two-separate-bridge-apps (confidence: high) ⭐⭐ ARCHITECTURE
**Context:** User corrected the framing — there are **TWO distinct Bridge Apps**, not one app with two jobs, and they must stay separate ("don't mix them").
**Learning:** **Bridge #1 = MenuLink orders → Punnelifosys POS (order injection).** Canonical home = `bridge-app/` on **origin/main** (PUBLIC), **Pos-only** (`Models/`, `Pos/RzrzPosAdapter`+`KitchenPrinter`, `Worker.cs`, `Services/SupabaseService.cs`, `Program.cs` — NO `DigitalInvoice/`). Core loop (`pos_outbox` → `InsertInvoice`, HoldMode default) is BUILT + prod-tested to v2.7. Missing = the 3 "always-on" workers: **heartbeat sender, invoice-status poller, POS items-catalog sync** ("APIs ready, .NET code needed"). **Bridge #2 = WhatsApp digital-invoice sender for the POS** — lives ONLY in the LOCAL `D:\menulink-bg` worktree on `feat/background-whatsapp-invoice-2`, which ADDED `DigitalInvoice/` (24 files) into the same csproj = where the two got physically mixed. **For Bridge #1 work use the clean `D:\menulink\bridge-app` (main), NOT the bg worktree.** ⚠️ main's `Pos/*.cs` DIFFER from bg's `Pos/*.cs` (both diverged) — reconcile before porting anything between them.
**Source:** session:2026-07-03 — user correction + code read
**Triggers:** two bridge apps, Bridge #1 order injection, Bridge #2 WhatsApp invoice, don't mix, bridge-app main Pos-only, DigitalInvoice bg worktree, heartbeat status-poll items-sync missing

### LRN-2026-06-28-unsplash-fruit-drink-queries-return-alcohol (confidence: high) ⭐⭐ CRITICAL (halal)
**Learning:** When gap-filling photos from Unsplash for a Saudi tenant, queries like `"<fruit> drink"`, `"<fruit> cocktail"`, `"hibiscus berry red drink"`, `"mojito"` frequently return **alcoholic cocktails with a visible liquor bottle** (e.g. a `«1800»` tequila bottle showed up on 3 Wadi Almusafir hibiscus/كركديه items). Also `"hibiscus"` alone returns literal **flowers**, not a beverage. **ALWAYS build a montage of every gap-filled drink photo and eyeball for bottles/flowers/wrong-form before publish** (the same montage step that catches the name-collision in [[LRN-2026-06-28-name-collision-wrong-photo]]). Safer query patterns: append `glass`/`iced tea`/`mocktail` and avoid `cocktail`; for كركديه use `"red iced tea glass mint"` / `"karkade red tea"`; reuse ONE verified halal image across flavor variants rather than per-flavor fetches. Fix = re-fetch 3 candidates (`per_page=3`), montage, pick the clean one, apply to all variants. Tooling: the per-tenant gapfill script pattern in `scripts/coffee-secret-unsplash-gapfill.mjs` + a sharp montage.

### LRN-2026-06-15-bg7-zero-touch-proven-on-clone (confidence: high) ⭐⭐ MILESTONE
**Context:** After fixing the helper DLL (see next entry), **two REAL POS sales on the clone** (BillNo 6 = 53.00 SendOnly, BillNo 7 = 16.00 SendAndPrint) auto-delivered the WhatsApp invoice — no touch, no crash, no WhatsApp Desktop. Full BG-7 zero-touch.
**Learning:** End-to-end chain works: POS `frmPayment` (`EnableDigitalInvoiceSend=1` + `DigitalInvoiceSendTransport=BackgroundBridge` in `PunnelifosysResApp.exe.config`) writes a `SendJob` JSON to `C:\ProgramData\PunnelifosysDigitalInvoice\spool\incoming` on Pay → the co-hosted Bridge sweeps it → renders from `client` DB → Meta `invoice_ready` template → **AcceptedByMeta** with a real wamid → arrives on the handset. Both **SendOnly and SendAndPrint** write a spool (PrintOnly does not). Outbox = SQLite at `C:\ProgramData\PunnelifosysDigitalInvoice\data\invoice-sender.db` (tables `send_jobs`, `send_attempts`, `status_events`, `service_state`) — **no `sqlite3` CLI on this box; query with `python` (Python312) `import sqlite3`**. Helper writes `idempotencyKey = {invoiceId}:SEND_DIGITAL_INVOICE` (dedupe backstop). A stale `FailedPermanent` row can linger (the 2026-06-13 job that hit "API access blocked") — harmless history, not a today-failure; read `send_attempts.result`/`last_error` to tell stale from fresh. **Number mapping confirmed for these invoices:** `Invoice.InvoiceNo` = small per-day-ish (6/7, the bag-icon number) and `Invoice.BillNo` = the big running number (33937/33938, the "رقم الفاتورة" line) — the render shows bag=InvoiceNo, "رقم الفاتورة :"=BillNo (matches the approved parity). **Cost model (production, real number):** in an OPEN 24h window → free (service msg or utility template); OUT of window → **paid utility template** (KSA rate) — till invoices are usually out-of-window → paid unless the customer messages first. Cloudflare gateway = what enables the free in-window path + delivery/read receipts (deferred; gateway-less works for sending). REMAINING for production: run Bridge as a Windows Service, wire Cloudflare gateway, ROTATE the in-transcript Meta token into DPAPI.
**Source:** session:2026-06-15 — BG-7 zero-touch live on clone
**Triggers:** BG-7 zero-touch, BackgroundBridge live, spool incoming, AcceptedByMeta wamid, SendOnly SendAndPrint, invoice-sender.db, send_jobs send_attempts, python sqlite3, idempotencyKey SEND_DIGITAL_INVOICE, stale FailedPermanent, InvoiceNo vs BillNo, cost in-window free out-of-window paid utility, Bridge windows service

### LRN-2026-06-15-helper-dll-wrong-assembly-name-crashes-frmpayment (confidence: high) ⭐⭐ CRITICAL
**Context:** First real BackgroundBridge Pay on the clone POS (ZKDebug) → the **whole POS crashed** the instant `frmPayment` opened (red blank form, app closes). Windows Event Log (.NET Runtime, id 1026) = `System.IO.FileLoadException at Punnelifosys.ResApp.UI.frmPayment.frmPayment_Load`.
**Learning:** The deployed helper `ZKDebug\PunnelifosysResApp.DigitalInvoice.dll` was a **broken/obfuscated build whose ASSEMBLY NAME was `diNEW_662579019`** (not `PunnelifosysResApp.DigitalInvoice`), so the patched EXE — which references the helper by simple name in `frmPayment_Load` (`AttachCombo`) — couldn't bind → `FileLoadException` → unhandled → crash. **Diagnose:** `[Reflection.AssemblyName]::GetAssemblyName(dll).Name` must equal `PunnelifosysResApp.DigitalInvoice`. (`Assembly.LoadFrom` SUCCEEDS on the bad DLL because LoadFrom binds by **path** not name — it hides the bug; the EXE binds by name. Non-strong-named ⇒ version is ignored, only the simple name must match.) **Fix = rebuild from source with the correct name:** `C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe /target:library /out:PunnelifosysResApp.DigitalInvoice.dll DigitalInvoice.cs` (+refs System, System.Core, System.Configuration, System.Data, System.Drawing, System.Windows.Forms, System.Xml). csc sets assembly name = the `/out` filename. BEFORE deploy: verify `.Name`, reflect that the public hooks exist (`AttachCombo`, `RefreshComboLanguage`, `SendModeHook`, `GetSendTransport`, `WriteSpoolJob`), backup the broken DLL, then copy in (POS must be closed — no file lock). The on-disk `bin\` helper (hash E6B7C531) is the OLDER **manual-only (1B)** build (pre-BackgroundBridge 2026-06-09 source) — do NOT restore it for zero-touch, it lacks `WriteSpoolJob`; **rebuild instead.** ZKDebug is Samer's dev/test copy (free to modify). Deployed good DLL hash = `4C4E4964…`.
**Source:** session:2026-06-15 — BG-7 first live Pay crash + fix
**Triggers:** frmPayment crash, FileLoadException, diNEW_662579019, wrong assembly name, AttachCombo, helper DLL deploy, csc rebuild Framework v4.0.30319, GetAssemblyName Name, LoadFrom hides bug, ZKDebug, Event Log 1026 .NET Runtime, E6B7C531 manual-only

### LRN-2026-06-15-meta-api-access-blocked-is-personal-account-verify (confidence: high) ⭐⭐ CRITICAL
**Context:** Resuming the background-invoice live test, EVERY Graph call (incl. `/me` and `/debug_token`) returned `OAuthException code 200 "API access blocked"` on the permanent System-User token (menulink-bridge, id 122107461021353235). Same wall as 2026-06-13 — now confirmed recurring.
**Learning:** `code 200 "API access blocked"` = a **personal Facebook account verification hold (confirm email + phone)** on the account behind the app — NOT the token, NOT the closed 24h window, NOT Business verification, NOT any business asset. Tell-tale: even `/me` fails (a window/template distinction is irrelevant when `/me` itself is blocked). **Diagnosis maze to AVOID** (cost ~5 navigation rounds): Business Settings → Security Center offers a 2–5d *Business* verification (wrong); `accountquality` / `business-support-home` redirect to CLEAN business-asset pages that only ever show asset issues, never the personal hold; Accounts Center → Contact info just *lists* contacts (listed ≠ verified). **The prompt that actually fixes it appears when you open `developers.facebook.com` directly** → forces "confirm your email + phone" → complete it → API restored within minutes (and any paused template review resumes). After it clears, re-run `GET /me` to confirm (returns the system-user name/id).
**Source:** session:2026-06-15 — live test resume; user cleared via the developers.facebook.com prompt
**Triggers:** API access blocked, code 200, OAuthException, /me blocked, personal account verification, confirm email phone, developers.facebook.com prompt, accountquality redirect, Security Center business verification not the cause, System user token menulink-bridge

### LRN-2026-06-15-bg7-bridge-send-proven-gateway-less (confidence: high) ⭐⭐
**Context:** First end-to-end LIVE send from the Bridge PROGRAM itself (not a manual curl) on the clone laptop DESKTOP-KUT35C6 — `DigitalInvoiceSenderWorker` imported a spool, rendered invoice 33931 from DB `client`, sent template `invoice_ready` → received on owner handset. Log: `DigitalInvoice processed 1 job(s); outbox: AcceptedByMeta=1`.
**Learning:** The Bridge runs in **gateway-less mode** when `DigitalInvoice:GatewayUrl` is empty → `StubWindowStateProvider(false)` → window treated CLOSED → **approved-template path** (proven out-of-window delivery), skipping Cloudflare/status-sync. So the WHOLE pipeline is testable with NO Cloudflare Worker. Required knobs: **`DigitalInvoice:AllowPaidTemplate=true`** (else `MetaCloudTransport` returns `BlockedByPolicy` when window closed); `Enabled=true`; `InstallationId` (any non-empty — also the DPAPI token key); `PhoneNumberId=1214915411698815`; `UtilityTemplateName=invoice_ready`/`Lang=ar`; **`GraphVersion=v25.0`** (DigitalInvoiceOptions default is the stale `v21.0` — override it). Config without editing files = env `MENULINK_BRIDGE_DigitalInvoice__<Key>=...` (prefix `MENULINK_BRIDGE_`, `__`=`:`). Token → DPAPI via `set MENULINK_BRIDGE_META_TOKEN=...; dotnet run -- store-token <InstallationId>` (roundtrip-verified, never in config/log). Feed a job by dropping a `SendJob` JSON in `<SpoolRoot>\incoming\` (fields in `SendJob.cs`; minimal = jobId + invoiceId GUID + billNo + customerPhoneE164 + language). Invoice 33931 GUID = `9C89D502-2B04-421D-92E6-F7D6AE4A34C7` (InvoiceNo=1, BillNo 33931, type 0 TakeAway). Run host `--no-build` in background, watch `AcceptedByMeta=1`, then stop. Pos conn must be `Server=.;...;User Id=<LOCAL_SQL_USER>;Password=<LOCAL_SQL_PASSWORD>` ([[LRN-2026-06-12-bg1-render-verified-on-clone]] — appsettings.json still wrongly says `.\SQLEXPRESS` + Integrated Security). NOT yet exercised: the helper-DLL writing the spool on real Pay (Level B / BG-7 zero-touch).
**Source:** session:2026-06-15 — BG-7 Bridge-level proof on clone
**Triggers:** BG-7, gateway-less, GatewayUrl empty, StubWindowStateProvider, AllowPaidTemplate, BlockedByPolicy, store-token DPAPI, MENULINK_BRIDGE_ env override, SendJob spool incoming, AcceptedByMeta, GraphVersion v25 default v21, invoice 33931 GUID, DigitalInvoiceSenderWorker

### LRN-2026-06-12-meta-cloud-send-live-and-render-parity (confidence: high) ⭐⭐ CRITICAL
**Context:** First LIVE WhatsApp send for the background-invoice project via Meta Cloud API (dev mode, test number), then reworked BG-1 to match the real POS receipt. App "MenuLink Invoices" (App ID 1959853721317258) under portfolio "MenuLink" (business_id 2854161678281816), acct id.menulink@gmail.com; test sender +1 555 652 4050 / Phone-Number-ID 1214915411698815 / WABA 1012069774676048.
**Learning (Meta send):** A brand-new FB account hits **"Facebook Account Too New To Create A Business"** — wait it out or use an established account; portfolio creation is the wall, not app creation. Test-number sends require THREE things: (1) recipient added **AND WhatsApp-code-verified** (just adding → `#131030 not in allowed list`); (2) **free-form messages (image/text) need an OPEN 24h customer window** — open it by messaging the test number FROM the recipient phone; **templates** (e.g. `hello_world`) deliver with NO window, so "template arrives but image doesn't" = window closed; (3) media upload must set **`image/png` on the multipart file part** — PowerShell `Invoke-RestMethod -Form` tags it `application/octet-stream` → `#100`; use **`curl.exe -F "file=@path;type=image/png"`**. Graph base = `https://graph.facebook.com/v25.0`. Pilot needs NO business verification (dev mode + tester role); verification/template/permanent-token are only for production scale. Temp tokens are 24h — mint a permanent System-User token (DPAPI) for real use, never paste in chat.
**Learning (render-parity):** The digital invoice MUST reproduce the POS-printed receipt (owner rule [[feedback_use_client_provided_design]]) — a clean redraw was rejected. Reference = the POS "print to PDF" output. The header (name/address/phone/thanks/currency/VAT-reg) **and the logo ARE in `dbo.GeneralSettings`** (`Company_A`,`Address_A`,`Address1_A`,`Phone_A`,`Thanks_A`,`CurrencyA`='ريال',`TaxReg`,`Tax`,`CompanyLogo_A` = SQL `image` blob) — the old BG-1 audit claim "not in the DB, config only" was WRONG. `InvoiceDataLoader.LoadCompany()` now pulls them (logo re-encoded to PNG via System.Drawing). Layout to match: logo → centered header → order-type **icon**(bag=TakeAway type 0) + big InvoiceNo → "رقم الفاتورة : "+BillNo → items table `# / الصنف / الكمية / السعر / الإجمالي` + **مجموع** row → left-aligned الخصم/الإجمالي(big)/المقبوض/المتبقي → صافي الفاتورة قبل الضريبة / ضريبة القيمة المضافة (15%) / الدفع - نقدي → وقت الفاتورة + المطبوعة بواسطة → thanks → ZATCA QR. Prices `0.##`, totals `N2`. To inspect a POS PDF use pymupdf (`fitz`) — poppler/pdftoppm NOT installed; embedded logo/icon extractable via `page.get_images`.
**Source:** session:2026-06-12 — Meta dev-mode live send + BG-1 18.pdf parity rework (uncommitted: 5 files + assets/order_takeaway.png)
**Triggers:** Meta Cloud API, WhatsApp send, test number, 131030 allowed list, 24h window, free-form vs template, hello_world, media image/png 100, curl multipart, account too new, dev mode, GeneralSettings header logo CompanyLogo_A, render parity, 18.pdf, LoadCompany, order-type bag icon, pymupdf fitz, InvoicePdfRenderer

### LRN-2026-06-12-bg1-render-verified-on-clone (confidence: high) ⭐
**Context:** While the Meta app was on a new-account hold, re-ran the BG-1 background renderer against real clone data on DESKTOP-KUT35C6 to confirm the exact image the background WhatsApp path will send. Rendered live invoice 33931 (`dotnet run -- render-invoice <InvoiceID-GUID> ar|en [outDir]`).
**Learning:** **BG-1 ✅ verified on real data** — AR + EN both correct (VAT-inclusive totals 34.78+5.22=40.00, ZATCA QR present, clean RTL/LTR layouts; output in `docs/proofs/bg1-render/`). Two operational gotchas on the clone laptop: (1) the SQL instance is the **DEFAULT** instance (`Server=.` / `MSSQLSERVER`), **NOT** `.\SQLEXPRESS` as `bridge-app/src/MenuLink.BridgeApp/appsettings.json` states, and **Integrated Security FAILS** (`DESKTOP-KUT35C6\USER` can't open `client`) → must use **sa / <LOCAL_SQL_PASSWORD>**. Override at runtime without editing the file via env var **`MENULINK_BRIDGE_Pos__ConnectionString`** (config prefix `MENULINK_BRIDGE_`, key `Pos:ConnectionString`, `__`=`:`). (2) The `dd MMM yyyy` date renders correctly in EN ("09 Jun 2026") but RTL-bidi reorders it in AR ("Jun 2026 09") — cosmetic; owner chose to KEEP it as-is. Clone invoice 33931 has no persisted Phase-2 QR so the renderer fell back to **Phase-1 TLV** (the reuse-don't-re-sign path is still untested — needs an invoice with a non-null `ZatcaReportingDetails.QRCode`).
**Source:** session:2026-06-12 — BG-1 re-verify on clone
**Triggers:** render-invoice, BG-1, background renderer, MENULINK_BRIDGE_, appsettings SQLEXPRESS wrong, default instance MSSQLSERVER, sa <LOCAL_SQL_PASSWORD>, integrated security fails client, dd MMM yyyy RTL date, Phase-1 TLV fallback, proofs/bg1-render

### LRN-2026-06-06-pos-decompiled-graphed-and-skilled (confidence: high) ⭐⭐ STRATEGIC
**Context:** Samer's full POS was decompiled (ILSpy) into `D:\Samer\research-output\decompiled\` (481 C# files, Branch + Server) with 5 research write-ups (`0*.md`). This session ran graphify over it (AST all 481 files + size-aware semantic extraction on 12 high-signal areas → `D:\Samer\research-output\graphify-out\`: `graph.json` 7,463 nodes, `graph-semantic.html`, `GRAPH_REPORT.md`) and built two new skills.
**Learning:** **The POS is now a first-class, source-grounded knowledge base — stop guessing about RzRz internals.** New skills: **`punnelifosys-pos`** (deep app reference: architecture, the DAL→284-sproc contract, ZATCA, printing, sync, schema, source-map) and **`punnelifosys-feature-dev`** (playbook for building features INTO the co-owned WinForms source — the sanctioned exception to "never modify the POS source"). This `menulink-integration` skill stays the *integrate-around-it* skill. Corrected stale facts while enriching: it's **stored-procedures + ADO.NET, not EF**; the `Items` columns are **`ItemName_E`/`ItemName_A`** (not `Item`/`Item_A`), there's **no verified `IsActive`** (real items = `ItemParent <> 0 AND ItemName_A <> '-'`); the InvoiceType enum is the authoritative `GetInvoiceType` set (0..11, **11=Online**); fixed the `sql-patterns.md` TypeScript example (it used the broken `<Items>` wrapper + `TaxPercent=15` + type 5/1 that the doc's own prose warns against).
**Source:** session:2026-06-06 — decompile + graphify + skill-creator
**Triggers:** punnelifosys-pos, punnelifosys-feature-dev, decompiled POS, graphify, knowledge graph, CommonBusiness, CommonDAL, GetInvoiceType, feature-dev, ItemName_A, source of truth

### LRN-2026-06-06-live-schema-extracted-verified (confidence: high) ⭐⭐
**Context:** The dev laptop `DESKTOP-KUT35C6` (SQL Server 2022) runs the live, licensed Branch app with a fresh DB copy literally named **`client`** = RZRZ Bukhari / Almalez **branch 2** (33,897 invoices, latest 2026-06-05; `sa`/`<LOCAL_SQL_PASSWORD>`, `accreef` also local). The app binaries are **byte-identical (SHA256)** to our decompiled snapshot (build 2026-01-07) → code not stale. Extracted the full schema read-only → `menulink-vault\_extract\schema_client.json` (94 tables / 8 views / 434 procs+fns / **0 FKs**) + live proc bodies `client_proc_*.sql` + reusable `extract_client.py`.
**Learning:** Schema is now ✅ **verified** (punnelifosys-pos/domain-and-schema.md + menulink-data updated). Corrections vs the old inferred docs: (1) **0 FKs and almost no declared PKs** — every big transactional table (`Invoice`, `InvoiceDetails`, `KitichenOrderForPrint`, `PaymentDetails`, `OnlineCustomer`, `GeneralSettings`…) uses de-facto GUID/identity keys; (2) `Invoice` has **NO `IGUID`** — cancel→original linkage is the separate **`CancelInvoice.InvoiceID`** table; ZATCA credit-note ref is **`ZatcaInvoice.ReferenceEInvoiceId`**; `IGUID` exists only on `ZatcaIntegrationSettings`; (3) **`AppendInvoiceIDS` is an `InsertInvoice` param, not a column**; (4) `InvoiceDetails` row id = **`InvoiceDetailID`** (not "SaleItemID"), qty = **`Quantity`**; real tables carry a `Details` suffix (`CounterDetails`/`CustomerDetails`/…); printer table = **`ItemPrinters`**; (5) `GeneralSettings` = **82 cols** (branch-variant; 80 on Aziziah); (6) InvoiceType enum re-confirmed (11=Online). **Branch-invariance confirmed:** core tables identical vs Aziziah; only `GeneralSettings` differs. ⚠️ **The live `InsertInvoice` body changed** vs the 2026-05-28 vault copy (862 vs 1723 lines, 0.66 similarity) — the runtime-captured XML contract in `sql-patterns.md` is still authoritative for INPUT, but re-verify if you touch proc internals; live body is in `client_proc_InsertInvoice.sql`.
**Source:** session:2026-06-06 — read-only live extract from `client@DESKTOP-KUT35C6`
**Triggers:** schema_client.json, verified schema, 0 FKs, no IGUID, CancelInvoice, AppendInvoiceIDS, ItemName_A, InvoiceDetailID, GeneralSettings 82, DESKTOP-KUT35C6, client DB, accreef local, branch-invariance, InsertInvoice changed

### LRN-2026-06-06-operating-the-live-pos (confidence: high) ⭐
**Context:** Drove/observed the live `PunnelifosysResApp.exe` on `DESKTOP-KUT35C6` (admin / **<POS_PIN>**, DB `client` = disposable copy) to reverse-engineer runtime behaviour. Built the new **`punnelifosys-pos-operate`** skill (toolkit + screen maps + scripts).
**Learning:** Three runtime observation techniques (all read-only, work in any session): (1) **XE SQL trace** on `client` = ground-truth capture of every sproc the app fires — `scripts/start_trace.sql` + `read_trace.py`; this is the way to learn/verify any flow + what menulink must replicate. (2) **UIA read-state** — but `pwsh` is **MTA** so UIA returns 0; run via **`powershell.exe -STA`** + set **per-monitor-DPI-aware**; the app is **custom-drawn** so every control is a `Pane` (no InvokePattern) yet exposes Name + screen-rect + value. (3) **Screenshot** via `CopyFromScreen` (**`PrintWindow` returns black** for this app); foreground first via the AttachThreadInput trick. ⚠️ **CRITICAL:** synthetic **input is BLOCKED** in the agent/sandbox session — `SetCursorPos` returns `False`, and `PostMessage(WM_LBUTTON*)`/`SendMessage(BM_CLICK)` to the (real WinForms) buttons don't fire. So you **cannot click/type autonomously from here**; to drive an order/payment either use an interactive console session OR (default) have the **user perform the action while the trace captures it**. `.ps1` files with Arabic literals must be **UTF-8 with BOM** or `powershell.exe` corrupts them.
**Source:** session:2026-06-06 — live-app driving attempt + operate-skill build
**Triggers:** operate POS, drive PunnelifosysResApp, run the app, XE trace, rzrz_trace, UIA STA MTA, SetCursorPos blocked, input injection, screenshot POS, admin <POS_PIN>, punnelifosys-pos-operate

### LRN-2026-06-06-live-order-flow-captured (confidence: high) ⭐⭐ CRITICAL
**Context:** User performed a real order (TakeAway sale + a delivery) on the live POS while the XE trace ran — captured the exact runtime sproc sequence. Full detail: `punnelifosys-pos-operate/references/captured-flows.md`.
**Learning:** Verified runtime flow (CounterID 6, UserID 1): **item-pick** = `CheckItemIsParent @ItemID` → (`GetItemsToBind @Category` if parent) / (`GetItemDetails @ItemID,@OnlineCustomerID` + `CheckCookItemQtyForSales @SaleItemID,@Qty` if leaf). **Order write** = `UpdateConfirmStatausofCancelAuthorization @InvoiceID='000…0'` (sic) then **`InsertInvoice`** (returns new InvoiceID). ⭐⭐ **PAYMENT IS A SEPARATE PROC:** `InsertInvoice` writes the order+kitchen on leaving the sales screen; **`InsertPaymentDetails`** records the payment at موافق (`@InvoiceID,@CounterID,@PaidAmount,@Discount,@GivenAmount,@ChangeAmount,@Cash,@Card,@OnlinePaymentType,@CardPaymentType,@Waiter,@Person,@OnlineBillNo,@TobaccoVatAmount,@InvoiceDiscountPercentage,@Mobile,@PointAmount`). So HoldMode = `InsertInvoice` only; full-pay = also `InsertPaymentDetails`. **Print** = `GetPrintersOfKitichenOrder[WithOutCancel]` + `GetItemsForPrintInvoice @Language`. **Delivery** = `GetCustomersForInvoice` (attach customer) → `InsertInvoice` → `GetAllDrivers`/`GetInvoiceToAssignDriver`/`GetInvoiceAssignedToDriver` → **`InsertDeliveryDetails @XmlDriverOrder`** (CustomerID+InvoiceID+DriverID). ✅ The live `@XmlInvoice`/`@XmlItems` shape **matches `sql-patterns.md`** (single `<Invoice/>`, `<Items/>` no wrapper, `TaxPercent="0"`); ⚠️ live cashier value has a stray leading **`N`** (`N'N<Invoice…'`) the proc tolerates.
**Source:** session:2026-06-06 — live XE trace `rzrz_trace` of a user-performed order+delivery
**Triggers:** InsertPaymentDetails, payment separate proc, InsertInvoice flow, item pick, CheckItemIsParent, GetItemsToBind, CheckCookItemQtyForSales, InsertDeliveryDetails, GetInvoiceToAssignDriver, leading N XML, captured-flows, runtime sproc sequence

### LRN-2026-06-06-dinein-table-append-settle (confidence: high) ⭐
**Context:** User performed a Family-section dine-in flow on the live POS, captured as 9 screenshots (`D:\BACK UP RZRZ\family-section-order-workflow\`; documented in `punnelifosys-pos-operate/references/captured-flows.md` Flow C). ✅ **Sprocs TRACE-VERIFIED 2026-06-07** (re-ran `rzrz_trace`) — corrected two earlier inferences.
**Learning:** **Dine-in = a table holds an accumulating order.** صالة العائلات/أفراد → floor plan (`frmFamilyTable`) → select table → **تنزيل طلبات** ("place order") = **`InsertInvoice @IsHold=0, @SectionID=2, @InvoiceType=6` (FamilyTable), `TableID=<n>`** (✅ NOT IsHold=1 — held on the table via `TableID` + `UpdateTableDetails @Status=2`, not IsHold) → kitchen ticket. **Additional order = re-open table (`GetInvoiceIDByTableID @TableID,@SectionID` loads the existing invoice) → `InsertInvoice` called again with the SAME `InvoiceID` (an UPDATE), re-sending the FULL item list** (old items keep TaxPercent=15, new items =0); **`@AppendInvoiceIDS` is EMPTY — append-to-table is a same-invoice update, NOT the AppendInvoiceIDS merge.** **تسديد (settle)** → `frmPayment` → موافق → `InsertPaymentDetails` → ZATCA simplified-invoice print → `UpdateTableDetails @Status=4` then `=0`. **Table status enum:** 0=free, 2=occupied, 4=paid. ⚠️ login `CheckUserValid @Password=N'<POS_PIN>'` is **plaintext** (security debt confirmed). Floor ops: دمج/فصل/نقل طاولة, حجز, الطاولة جاهزة. **Confirmed on the printed receipt:** prices are **VAT-INCLUSIVE** (total 89 = net 77.39 + 15% VAT 11.61; 89÷1.15=77.39); `Invoice` carries both **`InvoiceNo`** (global, e.g. 33902) and **`BillNo`** (daily counter, e.g. 5 — this is the "رقم الفاتورة" shown on the payment screen); VAT reg on receipt = `GeneralSettings.TaxReg` 311750526500003. **For menulink:** online orders should likely emulate the simple TakeAway/online path (not the table-hold path); dine-in append/settle is for in-house cashier use.
**Source:** session:2026-06-06 — 9 screenshots of a family-section dine-in order
**Triggers:** dine-in, table order, family section, صالة عائلات, frmFamilyTable, تنزيل طلبات, تسديد, settle, append order, AppendInvoiceIDS, BillNo vs InvoiceNo, VAT inclusive, ZATCA simplified invoice, QR receipt, SectionID 2
...
### LRN-2026-06-07-patch-pos-without-vs-or-dnspy (confidence: high) ⭐⭐ STRATEGIC
**Context:** Shipped Digital-Invoice 1B (Payment-Action combo + dup-payment guard) into `frmPayment` with NO Visual Studio, NO dnSpy on the box. Tooling present: Framework `csc.exe` (`...\Framework64\v4.0.30319\`), legacy Framework MSBuild, PEVerify (NETFX 4.8 Tools); `dnlib.dll` fetched from NuGet.
**Learning:** **You can extend this WinForms POS reproducibly without the IDE.** Pattern: (1) write a greenfield helper DLL with all logic, compile via `csc /codepage:65001 /platform:anycpu` (codepage for Arabic literals; AnyCPU pure-managed loads into the host regardless of bitness); (2) a `dnlib` script applies **linear** IL hooks to the .exe (add fields; insert at method head/after a call; rewrite a call-site by repurposing the instruction so branch targets survive). **Avoid hand-writing try/catch IL on a payment path** — push branching into self-defensive helper facades so host edits stay linear. Validate: PEVerify-diff (patched vs original = 0 new findings) + `RuntimeHelpers.PrepareMethod` JIT of edited methods + dnlib re-load assertion, all on a scratch copy. Deploy by file-copy to `C:\RZRZ-CODE\Branch-RES\ZKDebug` (the laptop clone). Artifacts: `C:\RZRZ-CODE\Branch-RES\DigitalInvoiceHelper\` (Patcher.cs, DigitalInvoice.cs, click.ps1). See [[digital-invoice-whatsapp-1b]].
**Source:** session:2026-06-07 Digital Invoice 1B
**Triggers:** patch POS, dnlib, no Visual Studio, no dnSpy, csc, PEVerify, JIT verify, frmPayment hook, helper DLL, IL injection, greenfield assembly

---

### LRN-2026-06-07-pos-laptop-is-clone-not-prod (confidence: high) ⭐ CRITICAL
**Context:** The skill/plan labeled `client@DESKTOP-KUT35C6` as "live" and warned never to test there. Owner clarified: the laptop's `C:\RZRZ-CODE\Branch-RES\ZKDebug` install + its `client@DESKTOP-KUT35C6` DB are a **disposable test clone** (counter "Adil-laptop"/ID 6) — prod `client` DBs live on the real restaurant tills. The `(localdb)\ProjectModels` snapshots (`rzrz-almalaz-2026-05-28`, `aziziah-rzrz-2026-5-27`) are OTHER branches and lack this machine's counter (can't log in).
**Learning:** **Never assert which `client` is prod vs clone — confirm with the owner.** On this laptop, test freely on `C:\...\ZKDebug` + `client@DESKTOP-KUT35C6` (sa/<LOCAL_SQL_PASSWORD>). Still `SELECT DB_NAME()` + back up `ZKDebug` (gate G) before changes. The LocalDB ProjectModels DBs are only good for read-only schema/sproc checks, not login-driven smoke tests on this box.
**Source:** session:2026-06-07 Digital Invoice 1B smoke test
**Triggers:** which client is prod, DESKTOP-KUT35C6, laptop clone, test DB, counter Adil-laptop, ZKDebug, never assert DB name

---

### LRN-2026-06-08-dnlib-il-patch-gotchas (confidence: high) ⭐⭐ TECHNICAL
**Context:** Completing Digital-Invoice 1C/1D — inserting a send-block call into `frmPayment.btnSave_Click` via the dnlib patcher kept failing in non-obvious ways.
**Learning:** Two dnlib traps when hand-building IL: (1) **`MemberRef` implements BOTH `IField` and `IMethod`** — so `operand is IField` is true even for a property getter (`call get_X`). Rebuild a captured load by dispatching on the **opcode** (`ldsfld/ldfld` vs `call/callvirt`), never the operand's runtime type, or you get "Opcode does not have a field operand". (2) **Inserting a block *before* an existing branch target silently skips it** — here `if (GoogleSheetSyncRequire==1)` is a `brfalse` that jumps straight to `Sales.InvoiceID = Guid.Empty`; our send block went in just before that `ldsfld Guid.Empty`, so on the sync-OFF path the brfalse leapt over it and nothing sent (payment still recorded → looked like a gating bug). Fix: after inserting, **re-point every branch (incl. switch tables) that targeted the anchor to the new first instruction**. Validate each patch with PEVerify-diff + `RuntimeHelpers.PrepareMethod` JIT — but neither catches *reachability*, so also dump the method skeleton (calls/branches/Guid.Empty/ret) and confirm a branch lands on your block. See [[digital-invoice-whatsapp-1b]].
**Source:** session:2026-06-08 Digital Invoice 1C/1D
**Triggers:** dnlib, IL patch, MemberRef IField IMethod, Opcode does not have a field operand, branch re-point, inserted block skipped, brfalse jumps over, PrepareMethod, reachability

---

### LRN-2026-06-08-touch-pos-whatsapp-send (confidence: high) ⭐⭐ PRODUCT
**Context:** The clipboard→Ctrl+V WhatsApp send worked on a keyboard but the POS is a **touch-only** screen — cashier can't press Ctrl+V.
**Learning:** For a touch POS, **open WhatsApp *Desktop* via `whatsapp://send?phone=&text=` (not `wa.me`/Web) and instruct the cashier to long-press the message box → Paste → Send.** WhatsApp Web in a browser BLOCKS image paste from the right-click/long-press menu (needs keyboard Ctrl+V); the native Desktop app allows long-press→Paste of a clipboard image. Keep the inline image, no cloud. Rejected cloud-link (upload PNG→Supabase→send link): storage grows (~120KB × send-invoices) and needs purging — not worth one fewer tap. Also: **Arabic WhatsApp captions** must DROP emoji (they URL-encode to `?`/`�` and break bidi order) and **prefix each line with RLM (U+200F)** to force RTL; and **MessageBox** needs `MessageBoxOptions.RtlReading | RightAlign` for Arabic popups. PNG receipt: reuse `PrintInvoiceForCustomerArabic` (+ `CompanyName_A`), NOT the English `PrintInvoiceForCustomer`. Mode tracking (print/send/both) = helper writes `dbo.DigitalInvoiceLog` per payment + view `dbo.vw_DigitalInvoiceModeCounts`.
**Source:** session:2026-06-08 Digital Invoice 1D touch UX
**Triggers:** touch POS, no keyboard, Ctrl+V, whatsapp:// desktop, long-press paste, WhatsApp Web blocks paste, RLM RTL Arabic message, MessageBoxOptions.RtlReading, PrintInvoiceForCustomerArabic, DigitalInvoiceLog

---

### LRN-2026-06-08-whatsapp-prefill-url-budget (confidence: high) ⭐ TECHNICAL
**Context:** Building a full itemized WhatsApp caption (order#, invoice#, items×qty, totals before/after VAT) for the Digital-Invoice send.
**Learning:** The `whatsapp://send?text=` prefill URL has a length budget (worked at ~2.1k, set SAFE≈4000 encoded). Two cost traps when the text is Arabic + emoji: **box-drawing separators (`━`) cost ~108 encoded chars PER line** (3 UTF-8 bytes × ~12 chars × %-encoding) — drop them, use blank lines; **Arabic chars ≈6 encoded each, emoji ≈12, RLM ≈9**. So **trim by `Uri.EscapeDataString(msg).Length`, NOT by item count** — show every item that fits, append "+N صنف آخر" only when over budget (the pasted IMAGE always has the full list). Build the rich text by calling the POS's own `GetItemsForPrintInvoice(@InvoiceID,@language)` (cols: InvoiceNo=order#, BillNo=invoice#, ItemName_A/Quantity/Amount, NetInvoiceAmountWithOutVat=before-VAT, TaxAmount, InvoiceNetAmount=after-VAT) — needs `Sales.InvoiceID` passed from the host (still valid before the 4178 clear). Verify emoji encoding at BUILD time (unit-assert the message has 🧾 and NO `?`/`�`).
**Source:** session:2026-06-08 Digital Invoice rich message
**Triggers:** whatsapp url length, prefill text limit, box-drawing separator cost, EscapeDataString trim, GetItemsForPrintInvoice columns, emoji encoding ?/replacement, rich invoice message

---

### LRN-2026-06-11-whatsapp-transport-coexistence (confidence: high)
**Context:** Background-invoice transport decision. Owner rejected per-client Meta business accounts and asked about WhatsApp Desktop/Web automation "like before"; Baileys/web.js ban risk explained (detection is behavioral + client-fingerprint — per-client numbers only shrink blast radius, and the banned number would be the client's real business number).
**Learning:** Decided path = Cloud API **Coexistence**: client keeps their existing WhatsApp Business app number, onboarded via Embedded Signup QR (~10 min). SA supported; needs WA Business app ≥2.24.17 + number ≥7 days active (1–2 months recommended). MenuLink one-time: Meta business verification (2–5 days) → app review (video evidence) → System-User token; `invoice_ready` template approved once per WABA. Pilot can run with the Meta app in **dev mode** (client FB account as tester) before app review. Coexistence numbers lose live-location; broadcast lists become read-only.
**Source:** session:2026-06-11 | customer:platform-wide
**Triggers:** WhatsApp transport, Coexistence, embedded signup, Baileys, ban risk, Meta business verification, tech provider, background invoice
...
### 2026-06-11 · Background-invoice external gates: Coexistence decided + Cloudflare deployed
- **Decision (owner):** transport = Cloud API **Coexistence** — each client keeps their own number; per-client Meta accounts rejected; Baileys declined after ban-risk briefing. Captured as [[lrn-2026-06-11-whatsapp-transport-coexistence]].
- **Cloudflare gate executed** (acct id.menulink@gmail.com): D1 `a6a0f92a-386a-4693-b560-b01f32a0728d` (EEUR) created + `0001_init` migrated; Worker uploaded; secrets set (real WEBHOOK_VERIFY_TOKEN → gitignored `.dev.vars`; META_APP_SECRET placeholder). Blocked only on one-time workers.dev subdomain registration (interactive).
- **Friction:** wrangler v4 has no `subdomain` command and auto-answers prompts in non-interactive shells — subdomain registration must be done by the user (interactive `wrangler deploy` or dashboard).
- **Next:** register subdomain + redeploy → Meta dev app + business verification (2–5d, start immediately) → Coexistence-onboard testbed number in dev mode → seed D1 rows → BG-7 matrix.
