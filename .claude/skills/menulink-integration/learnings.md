...
**Context:** Drove/observed the live `PunnelifosysResApp.exe` on `DESKTOP-KUT35C6` (admin / **1416**, DB `client` = disposable copy) to reverse-engineer runtime behaviour. Built the new **`punnelifosys-pos-operate`** skill (toolkit + screen maps + scripts).
...
**Triggers:** operate POS, drive PunnelifosysResApp, run the app, XE trace, rzrz_trace, UIA STA MTA, SetCursorPos blocked, input injection, screenshot POS, admin 1416, punnelifosys-pos-operate
...
**Learning:** **Never assert which `client` is prod vs clone — confirm with the owner.** On this laptop, test freely on `C:\...\ZKDebug` + `client@DESKTOP-KUT35C6` (sa/sa@123). Still `SELECT DB_NAME()` + back up `ZKDebug` (gate G) before changes. The LocalDB ProjectModels DBs are only good for read-only schema/sproc checks, not login-driven smoke tests on this box.
