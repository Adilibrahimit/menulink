...
Payment idempotency: **runtime-verified (operational) 2026-06-09** — a clone double-tap test (BillNo 33931) produced **exactly one `PaymentDetails` row**; trace showed `InsertPaymentDetails` fired **once** (the runtime `_paymentPersisted` UI guard blocked the second tap). The sproc script also shows UPSERT (`if not exists … insert … else update … where InvoiceID=@InvoiceID`, `…2.InsertPaymentDetails.sql:130/132/179`); the live sproc is `WITH ENCRYPTION` so its standalone UPSERT was not exercised in isolation (the UI guard fired first). Operational guarantee — a cashier double-tap cannot create a second payment — is **VERIFIED** (closes Codex correction #5).
